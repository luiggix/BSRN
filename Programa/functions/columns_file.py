...
# Escribimos el archivo para pruebas.
#df.to_csv("BSRN_TEST_FILE.csv")

# Leemos el archivo de pruebas
df = pd.read_csv("BSRN_TEST_FILE.csv")
mounth_ = "5"
base_sql = "XXX"
...
porcent_GH_CALC_GLOBAL=(abs(df["GLOBAL_Avg"]-df["GH_CALC_Avg"])/df["GH_CALC_Avg"])*100
dispersion = df["GH_CALC_Avg"]

df["dif_GH_CALC_GLOBAL"]=dif_GH_CALC_GLOBAL
df["porcent_GH_CALC_GLOBAL"]=porcent_GH_CALC_GLOBAL
df["dispersion"]=dispersion
df["porcent_2"] = 0.01*dispersion
...
stadistic = ["dif_GH_CALC_GLOBAL","porcent_GH_CALC_GLOBAL","dispersion"]
...
ultraviolet = ["UVBw","UVTEMP_Avg","UVSIGNAL_Avg"]
